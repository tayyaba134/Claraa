...
from simple_transformer import BidirectionalSimpleTransformer
from audio_viT import *
from cnn import *
from pann import *
...
from whisper import *
from modules import PositionalEncoding, LayerNorm, TransformerEncoderLayerBidirectional, BidirectionalCLARAEncoder
from loss import CLARALoss, CLIPLoss

from scheduler import CosineAnnealingWarmupRestarts
from utils.accuracy import BidirectionalAccuracy, accuracy
...
class CLARA(nn.Module):
	...
	def __init__(self, hparm, text_encoder:Optional[nn.Module]=None, audio_encoder:Optional[nn.Module]=None) -> None:
		...
		if self.text_encoder == None:
			self.text_encoder = BidirectionalSimpleTransformer(
		...
		self.text_transform = TransformerEncoderLayerBidirectional(
			in_channels=self.hparm.text_encoder_project,
			out_channels=self.hparm.output_dim,
			num_layers=self.hparm.text_encoder_layers,
			nhead=self.hparm.text_encoder_heads,
		...
		self.audio_transform = TransformerEncoderLayerBidirectional(
			in_channels=self.hparm.audio_encoder_project,
			out_channels=self.hparm.output_dim,
			num_layers=self.hparm.audio_encoder_layers,  
			nhead=self.hparm.audio_encoder_latent_heads,      
...
class PLCLARA(pl.LightningModule):
	def __init__(	self, 
		...
		self.acc_fn = BidirectionalAccuracy(cache_labels=True)
...
class LinearProbeCLARA(pl.LightningModule):
	def __init__(self, 
		...
		*args, **kwargs
		...
		self.classifier = TransformerEncoderLayerBidirectional([self.feature_extractor._hparams.output_dim, 512, 128, num_classes], dropout=dropout)
